  ...
  try {
    ...
    if (!ENV.RABBITMQ_URL) {
      throw new InternalServerError(
        'RabbitMQ_URL is not set for disaster notification consumer',
      );
    }

    disasterNotificationConsumerConnection = await amqp.connect(
      ENV.RABBITMQ_URL,
    );

    disasterNotificationConsumerChannel =
      await disasterNotificationConsumerConnection.createChannel();
    logger.info(
    ...
    disasterNotificationConsumerConnection.on('error', (error: Error) => {
      logger.error(
        '[DisasterNotificationConsumer] Connection error:',
        error.message,
      ); //
      ...
      if (
        !isDisasterNotificationConsumerStopping &&
    ...
    disasterNotificationConsumerConnection.on('close', () => {
      logger.warn(`[DisasterNotificationConsumer] Connection closed`);
      if (disasterNotificationConsumerConnection) {
        disasterNotificationConsumerConnection.removeAllListeners();
      }
      disasterNotificationConsumerConnection = null;
    ...
    await disasterNotificationConsumerChannel.assertQueue(queueName, {
      durable: true,
    });
    logger.info(
        ...
        if (msg) {
          ...
          try {
            ...
            if (!notiPlayload.socketId) {
              logger.error(
                `[DisasterNotificationConsumer] Received notification with missing socketID:`,
                error,
              );
    ...
    if (disasterNotificationConsumerConnection) {
      ...
      try {
        ...
        logger.error(
          `[DisasterNotificationConsumer] Error cleaning up listeners: `,
          e,
        );
      }
    }
...
export async function startDiasterNotificationConsumer() {
  ...
  if (ENV.RABBITMQ_URL && ENV.RABBITMQ_NOTIFICATION_QUEUE_NAME) {
    ...
    logger.warn(
      `[DiasterNotificationConsumer] RabbitMQ_URL or NotifcationQueueName is not configured. Consumer will NOT start...`,
    );
  }
}
...
export async function stopDisasterNotificationConsumer() {
  ...
  if (ch) {
    try {
      ...
      logger.error(`[DiasterNotificationConsumer] Erorr closing channel: `, e);
    }
  }
  if (conn) {
    try {
      ...
      logger.info(`[DiasterNotificationConsumer] Error closing connection:`, e);
    }
  }
  logger.info(
    `[DiasterNotificationConsumer] Consumer stopped and connections attempted to close`,
  );
}
