...
export async function triggerTestNotificationForLocation(
  quakeLat: number,
  quakeLon: number,
  ...
  logger.info(
    `[DebugNotiService] Triggering test notification for quake at ${quakeLat}, ${quakeLon}`,
  );
  ...
  if (nearbySocketIds.length === 0) {
    const message =
      '[DebugNotiService] No active users found within the 200km radius.';
    logger.info(message);
    return { message, notifiedCount: 0 };
  }

  logger.info(
    `[DebugNotiService] Found ${nearbySocketIds.length} user(s) to notify.`,
  );
  ...
  return {
    message,
    notifiedCount: nearbySocketIds.length,
    notifiedSocketIds: nearbySocketIds,
  };
}
