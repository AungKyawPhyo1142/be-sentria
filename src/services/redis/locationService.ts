...
export async function updateUserLocation(
  ...
  try {
    ...
    logger.error(
      `[LocationService] Error updating user location for socketID: ${socketId}: `,
      error,
    );
  }
}
...
export async function removeUserLocation(socketId: string): Promise<void> {
  try {
    ...
    logger.info(
      `[LocationService] Removed location for disconnected socketID: ${socketId}`,
    );
  } catch (error) {
    logger.error(
      `[LocationService] Error removing user location for socketID: ${socketId}: `,
      error,
    );
  }
}
