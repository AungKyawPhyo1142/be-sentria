...
export async function deleteCommentReply(commentId: string, user: User){
  try{
    ...
    if(!commentReply){
      throw new NotFoundError(`Comment reply with ID of ${commentId} not found in mongoDB`);
    }
    ...
    if(!isCommentReplyOwner && !isPostOwner){
      throw new Error('Unauthorized: Only comment reply owner or post owner can delete this comment reply');
    }
