...
import { closeRabbitMQConnection, initRabbitMQConnection, publishToQueue } from "@/libs/rabbitmqClient";
import { getRedisClient, initRedisConnection } from "@/libs/redisClient";
import logger from "@/logger";
import axios from "axios";
import cron from 'node-cron'

const USGS_PAST_HOUR_URL = 'https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_hour.geojson';
...
async function checkForNewEarthquakes() {
    ...
    try {
        ...
        const response = await axios.get(USGS_PAST_HOUR_URL)
        ...
        for (const earthquake of earthquakes) {
            ...
            if (nearbySocketIds.length > 0) {
                logger.info(`[USGSPoller] Found ${nearbySocketIds.length} potentially affected user(s) nearby`)
...
async function startPollingService() {
    ...
    try {
        ...
        cron.schedule(`*/${POLLING_INTERVAL_MINUTES} * * * *`, checkForNewEarthquakes)

        logger.info(`[USGSPoller] Severice started. CRON job scheduled to run every ${POLLING_INTERVAL_MINUTES} minute(s).`)
        ...
        logger.error(`[USGSPoller] CRITICAL: Failed to init dependencies. Poller will not start`)
        process.exit(1)
    }
}
